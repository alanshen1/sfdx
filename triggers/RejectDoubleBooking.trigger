trigger RejectDoubleBooking on Session_Speaker__c (before insert, before update) {
    ...
    List<Session__c> related_sessions = [SELECT ID, Date_and_Time__c from Session__c WHERE ID IN :requested_bookings.keySet()];
    for(Session__c related_session : related_sessions) {
        requested_bookings.put(related_session.Id,related_session.Date_and_Time__c);
    }
    //get related speaker sessions to check against
    List<Session_Speaker__c> related_speakers = [SELECT ID, Speaker__c, Session__c, Session__r.Date_and_Time__c from Session_Speaker__c WHERE Speaker__c IN :speakerIds];
    ...
    for(Session_Speaker__c requested_session_speaker : trigger.new) {
        ...
        for(Session_Speaker__c related_speaker : related_speakers) {
            if(related_speaker.Speaker__c == requested_session_speaker.Speaker__c &&
               related_speaker.Session__r.Date_and_Time__c == booking_time) {
